...
public:
	...
	int ReadString(const char* pSection, const char* pKey, const char* pDefault, char* pBuffer, size_t szBufferSize)
		{ JMP_THIS(0x528A10); }
	int OriginReadString(const char* pSection, const char* pKey, const char* pDefault, char* pBuffer, size_t szBufferSize)
	{
		static unsigned long jmp_addr = 0x528A15;
		EPILOG_THISCALL;
		_asm {
			sub esp, 0x0C
			xor eax, eax
...
#define INI_READ(item, addr) \
	...
	template <size_t Size>
	constexpr int OriginReadString(const char* pSection, const char* pKey, const char* pDefault, char(&pBuffer)[Size])
	{
		return this->OriginReadString(pSection, pKey, pDefault, pBuffer, Size);
	}
